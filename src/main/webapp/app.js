// const baseUrl = `http://localhost:8080/mss/resources`;
...
function getLandingPageContent() {
    $.getJSON(`${baseUrl}/api/getLandingPageContentDetailsAll`, function (data) {
        if (data.success) {
            ...
            $('#pensionerImage').html(`<img src="./FileHandler?file=${pensionerImage}">`);
            ...
            let geo = obj.address.geo;
            let mapUrl = `https://www.google.com/maps/embed/@${geo.lat},${geo.lng},12z`;
        }
        getActiveConfig();
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    // $('#gif').css('display', 'none');
                    // $('#msg').css('display', 'block');
                    // $('#msg').html(`<h4>A password reset link has been sent to your email</h4>`);
                    // setTimeout(function () {
                    //     window.location.href = `./`;
                    // }, 5000);
...
function err(msg = `Error encountered, please try again`) {
    // $('#gif').css('display', 'none');
    // $('#msg').css('display', 'none');
    // $('#msg').html(`<h4>${msg}</h4>`);
    // setTimeout(function () {
...
function saveEmailTemplate() {
    ...
    let category = getSelectInputVal('category');
...
function getEmailTemplateEnums(callBack) {
    $.getJSON(`${baseUrl}/api/getEmailTemplatesEnum`, function (obj) {
        if (obj.success) {
            ...
            for (let i = 0; i < data.length; i++) {
                let temp = data[i];
                content += `<option value="${temp.category}">${temp.title}</option>`
            }
