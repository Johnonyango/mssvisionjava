...
public class FundMasterClient {
    ...
    public ReceiptDTO getContributionsReceiptsPerSponsor(long schemeId, String sponsorId) {
        try {
            Response response = target.path(APICall.GET_SPONSOR_CONTRIBUTIONS_RECEIPT_ETL)
                    .path(String.valueOf(schemeId))
                    .path(String.valueOf(sponsorId))
                    .request(MediaType.APPLICATION_JSON_TYPE)
                    .headers(myHeaders)
            ...
            s = s.replaceAll(",\n]}", "]}");
            Gson g = new Gson();
            ReceiptDTO fmListDTO = g.fromJson(s, ReceiptDTO.class);
            return fmListDTO;
        } catch (Exception e) {
    ...
    public FmListDTO bookContributionBill(BillRequestVM billRequestVM) {
        try {
            ...
            String s = response.readEntity(String.class);
            FmListDTO fmListDTO = new ObjectMapper().readValue(s, FmListDTO.class);
    ...
    public boolean addSponsorUser(SponsorUserVM sponsorUserVM) {
...
//            if (profile.equals("BILLING OFFICER")) {
//                profileName = profile.replace("BILLING OFFICER", "SPONSOR");
//            }
...
//                    .rows(billExceptionDTO.getRows()).build();
            ...
            String s = response.readEntity(String.class);
            MessageModelDTO messageModelDTO = new ObjectMapper().readValue(s, MessageModelDTO.class);
...
//            log.error(response.readEntity(String.class));
    ...
    public boolean checkClientIsETlSendSMSHasEmail(SMSVM smsvm, User user) {
        Config config1 = configRepository.getActiveConfig();
        if (config1.getClient().equals(Clients.ETL)) {
            try {
                if (user.getEmail().equals("") || user.getEmail() == null) {
                    sendSmsEtl(new SMSVM(smsvm.getPhoneNumber(), smsvm.getMessage()));
                    return false;
                } else {
                    String message = smsvm.getMessage() + "  More details have been sent on the email.";
                    sendSmsEtl(new SMSVM(smsvm.getPhoneNumber(), message));
                    return true;
                }
    ...
    public ResultsDTO getSponsorTPFA(long schemeId, long sponsorId, String receiptStatus) {
        try {
            ...
            s = s.replaceAll("},]", "}]");
            Gson g = new Gson();
            ResultsDTO resultsDTO = g.fromJson(s, ResultsDTO.class);
            return resultsDTO;
        } catch (Exception e) {
